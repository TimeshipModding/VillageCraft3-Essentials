package com.timeshipmodding.villagecraft3essentials.compat.jei.transfer;
...
public class WalletRecipeTransferHandler implements IRecipeTransferHandler<WalletMenu, RecipeHolder<CraftingRecipe>> {
    private static final IntSet PLAYER_INV_INDEXES = IntArraySet.of(0, 1, 3, 4);
    private final IRecipeTransferHandlerHelper handlerHelper;
    private final IRecipeTransferHandler<WalletMenu, RecipeHolder<CraftingRecipe>> handler;

    public WalletRecipeTransferHandler(IRecipeTransferHandlerHelper handlerHelper) {
        this.handlerHelper = handlerHelper;
        var basicRecipeTransferInfo = handlerHelper.createBasicRecipeTransferInfo(WalletMenu.class, null, RecipeTypes.CRAFTING, 1, 4, 5, 52);
        this.handler = handlerHelper.createUnregisteredRecipeTransferHandler(basicRecipeTransferInfo);
    }

    @Override
    public Class<? extends WalletMenu> getContainerClass() {
        return handler.getContainerClass();
    }

    @Override
    public Optional<MenuType<WalletMenu>> getMenuType() {
    ...
    @Nullable
    @Override
    public IRecipeTransferError transferRecipe(WalletMenu container, RecipeHolder<CraftingRecipe> recipe, IRecipeSlotsView recipeSlotsView, Player player, boolean maxTransfer, boolean doTransfer) {
