...
public class AmberCavesBlacklistCommand {
    public AmberCavesBlacklistCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("ambercaves").then(Commands.literal("blacklist")
                .then(
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        ...
        for (ServerPlayer player : targets) {
            if (!hideMessage) {
                player.sendSystemMessage(Component.literal("You have been blacklisted from " + groupColour + "The Amber Caves§r and can no longer teleport to " + groupColour + "The Amber Caves'§r Spawn!"), false);
            }

            if (ModList.get().isLoaded("luckperms")) {
                LuckpermsMethods.addGroup(player, ServerConfig.AMBERCAVES_BLACKLISTED_GROUP_NAME.get());
        ...
        context.getSource().sendSuccess(() -> Component.literal("You have blacklisted " + targetPlayerUsername + " from The Amber Caves and they can no longer teleport to " + groupColour + "The Amber Caves'§r Spawn!"), false);
        return 1;
    }
}