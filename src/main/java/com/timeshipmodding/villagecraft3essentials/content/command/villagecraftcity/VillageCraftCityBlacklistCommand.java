...
public class VillageCraftCityBlacklistCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        ...
        for (ServerPlayer player : targets) {
            if (!hideMessage) {
                player.sendSystemMessage(Component.literal("You have been blacklisted from " + groupColour + "VillageCraft City§r and can no longer teleport to " + groupColour + "VillageCraft City's§r Spawn!"), false);
            }
        ...
        context.getSource().sendSuccess(() -> Component.literal("You have blacklisted " + targetPlayerUsername + " from " + groupColour + "VillageCraft City§r and they can no longer teleport to " + groupColour + "VillageCraft City's§r Spawn!"), false);
        return 1;
    }
}