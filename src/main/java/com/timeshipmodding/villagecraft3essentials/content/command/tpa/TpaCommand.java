...
public class TpaCommand {
    public TpaCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("tpa").executes(p_137817_ -> execute(p_137817_, ImmutableList.of(p_137817_.getSource().getPlayerOrException())))
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> target) {
        ...
        if (player != null) {
            for (ServerPlayer targetPlayer : target) {
                TpaCommandManager.requestTpa(player, targetPlayer, "tpa");
                return 1;
            }
        }
