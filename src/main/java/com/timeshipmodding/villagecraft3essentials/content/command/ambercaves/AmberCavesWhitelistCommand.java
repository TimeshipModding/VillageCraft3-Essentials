...
public class AmberCavesWhitelistCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        ...
        for (ServerPlayer player : targets) {
            if (!hideMessage) {
                player.sendSystemMessage(Component.literal("You have been whitelisted from " + groupColour + "The Amber Caves§r and can now teleport to " + groupColour + "The Amber Caves§r Spawn!"), false);
            }
        ...
        context.getSource().sendSuccess(() -> Component.literal("You have whitelisted " + targetPlayerUsername + " from " + groupColour + "The Amber Caves§r and they can now teleport to " + groupColour + "The Amber Caves§r Spawn!"), false);
        return 1;
    }
}