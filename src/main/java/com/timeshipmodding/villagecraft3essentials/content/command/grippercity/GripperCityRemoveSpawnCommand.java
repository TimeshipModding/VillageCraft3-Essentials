...
public class GripperCityRemoveSpawnCommand {
    public GripperCityRemoveSpawnCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("grippercity").then(Commands.literal("removespawn")
    ...
    private int execute(CommandContext<CommandSourceStack> context) {
        ...
        if (spawn[3] != 0 && spawn[4] != 0) {
            ...
            savedData.setGripperCitySpawn(spawn1);

            if (ModList.get().isLoaded("bluemap") && ServerConfig.ENABLE_SETSPAWN_BLUEMAP_MARKER_CREATION.get()) {
                BlueMapMethods.removeSpawnMarker("gripper-city", player);
            }
