...
public class GripperCityBlacklistCommand {
    public GripperCityBlacklistCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("grippercity").then(Commands.literal("blacklist")
                .then(
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        ...
        for (ServerPlayer player : targets) {
            if (!hideMessage) {
                player.sendSystemMessage(Component.literal("You have been blacklisted from " + groupColour + "Gripper City§r and can no longer teleport to " + groupColour + "Gripper City's§r Spawn!"), false);
            }
        ...
        context.getSource().sendSuccess(() -> Component.literal("You have blacklisted " + targetPlayerUsername + " from " + groupColour + "Gripper City§r and they can no longer teleport to " + groupColour + "Gripper City's§r Spawn!"), false);
        return 1;
    }
}