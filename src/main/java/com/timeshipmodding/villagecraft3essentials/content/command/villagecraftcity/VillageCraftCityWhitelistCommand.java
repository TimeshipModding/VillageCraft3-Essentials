...
public class VillageCraftCityWhitelistCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        ...
        for (ServerPlayer player : targets) {
            if (!hideMessage) {
                player.sendSystemMessage(Component.literal("You have been whitelisted from " + groupColour + "VillageCraft City§r and can now teleport to " + groupColour + "VillageCraft City's§r Spawn!"), false);
            }
        ...
        context.getSource().sendSuccess(() -> Component.literal("You have whitelisted " + targetPlayerUsername + " from " + groupColour + "VillageCraft City§r and they can now teleport to " + groupColour + "VillageCraft City's§r Spawn!"), false);
        return 1;
    }
}