...
public class VillageCraftCityWhitelistCommand {
    public VillageCraftCityWhitelistCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("villagecraftcity").then(Commands.literal("whitelist")
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        for (ServerPlayer player : targets) {
            if (hideMessage) {
                player.sendSystemMessage(Component.literal("You have been whitelisted from VillageCraft City and can now teleport to VillageCraft City Spawn!"), false);
            }

            if (ModList.get().isLoaded("luckperms")) {
                LuckpermsMethods.removeGroup(player, "villagecraftcityblacklisted");
            }

            targetPlayerUsername = Objects.requireNonNull(player.getDisplayName()).getString();
        }

        context.getSource().sendSuccess(() -> Component.literal("You have whitelisted " + targetPlayerUsername + " from VillageCraft City and they can now teleport to VillageCraft City Spawn!"), false);
        return 1;
    }
}