...
public class AmberCavesBlacklistCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        for (ServerPlayer player : targets) {
            if (hideMessage) {
                player.sendSystemMessage(Component.literal("You have been blacklisted from The Amber Caves and can no longer teleport to The Amber Caves Spawn!"), false);
            }
        ...
        context.getSource().sendSuccess(() -> Component.literal("You have blacklisted " + targetPlayerUsername + " from The Amber Caves and they can no longer teleport to The Amber Caves Spawn!"), false);
        return 1;
    }
}