...
public class AmberCavesWhitelistCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        for (ServerPlayer player : targets) {
            if (hideMessage) {
                player.sendSystemMessage(Component.literal("You have been whitelisted from The Amber Caves and can now teleport to The Amber Caves Spawn!"), false);
            }

            if (ModList.get().isLoaded("luckperms")) {
                LuckpermsMethods.removeGroup(player, "ambercavesblacklisted");
            }

            targetPlayerUsername = Objects.requireNonNull(player.getDisplayName()).getString();
        }

        context.getSource().sendSuccess(() -> Component.literal("You have whitelisted " + targetPlayerUsername + " from The Amber Caves and they can now teleport to The Amber Caves Spawn!"), false);
        return 1;
    }
}