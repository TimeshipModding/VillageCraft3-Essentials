...
public class GripperCityBlacklistCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context, Collection<? extends ServerPlayer> targets, boolean hideMessage) {
        for (ServerPlayer player : targets) {
            if (hideMessage) {
                player.sendSystemMessage(Component.literal("You have been blacklisted from Gripper City and can no longer teleport to Gripper City Spawn!"), false);
            }
        ...
        context.getSource().sendSuccess(() -> Component.literal("You have blacklisted " + targetPlayerUsername + " from Gripper City and they can no longer teleport to Gripper City Spawn!"), false);
        return 1;
    }
}